...
- Make sure Spotify is running on your computer, run `python` or `python3` `spotify_client.py` to register a username on the server, and put that username in the the settings of the Spotify tile. It is recommended you use a somewhat **unique username** as to make it easy, it does not require a password but people can also re-use a common one. Thankfully it is impossible to get the usernames of other users...
- You will need to keep the Python client running to make sure the launcher syncs your Spotify status. 

The code of this server is no longer in the backends directory due to abuse of the server after me integrating it with this project, also I will publish a video at some point explaining the launcher's new features and why I close sourced the data backend of it... The rest of the launcher is, and will always remain open source...
